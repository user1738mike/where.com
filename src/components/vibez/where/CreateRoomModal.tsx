...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from '@/components/vibez/ui/dialog'
...
const CreateRoomModal = ({ open, onOpenChange, onRoomCreated }: CreateRoomModalProps) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!data?.room?.id) {
        toast.error('Room was created but no room ID returned. Please try again.')
        return
      }

      toast.success(`${roomType === 'private' ? 'Private' : 'Public'} room created!`)
      onRoomCreated(data.room.id)
      onOpenChange(false)
      resetForm()
    } catch (error: any) {
      console.error('Failed to create room:', error, JSON.stringify(error))
      const msg = error?.message || error?.context?.message || 'Failed to create room. Please try again.'
      toast.error(msg)
    } finally {
      setIsCreating(false)
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">Create Group Room</DialogTitle>
        </DialogHeader>
              ...
              >
                <div className="flex items-start gap-2 rounded-lg border border-border p-2 hover:bg-accent cursor-pointer">
                  ...
                  <Label htmlFor="public" className="cursor-pointer flex-1 m-0">
                    ...
                    <div className="text-sm text-muted-foreground">Anyone can join instantly</div>
                  </Label>
                </div>
                <div className="flex items-start gap-2 rounded-lg border border-border p-2 hover:bg-accent cursor-pointer">
                  <RadioGroupItem value="private" id="private" />
                  <Label htmlFor="private" className="cursor-pointer flex-1 m-0">
                    <div className="font-medium">🔒 Private Room</div>
                    <div className="text-sm text-muted-foreground">You approve who joins</div>
                  </Label>
                </div>
              </RadioGroup>
            </div>
