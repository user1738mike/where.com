...
import { Avatar, AvatarImage, AvatarFallback } from '@/components/vibez/ui/avatar'
...
const VideoTile: React.FC<{
  ...
  onClick?: () => void
  isPinned?: boolean
}> = ({ stream, name, avatarUrl, isMuted, isVideoOff, isLocal, isRoomOwner, participantCount, onClick, isPinned }) => {
  ...
  useEffect(() => {
    ...
    if (videoRef.current) {
      ...
      const attemptPlay = async (muted = false) => {
        try {
          ...
          await videoEl.play()
          if (muted && !isIOS) setTimeout(() => { try { videoEl.muted = false } catch {} }, 800)
        } catch (err: any) {
          if (!muted && err?.name === 'NotAllowedError') setTimeout(() => attemptPlay(true), 100)
        }
      }

      try { if (videoEl.readyState >= 1) attemptPlay() } catch {}
      const onLoaded = () => { videoEl.removeEventListener('loadedmetadata', onLoaded); attemptPlay() }
      videoEl.addEventListener('loadedmetadata', onLoaded)
      setTimeout(() => { if (videoEl.paused && videoEl.readyState >= 1) attemptPlay(true) }, 1000)
      setTimeout(() => { if (videoEl.paused && videoEl.readyState >= 1) attemptPlay(true) }, 2000)
    ...
    if (audioRef.current && !isLocal) {
      ...
      try { audioRef.current.muted = false; audioRef.current.play() } catch (err) { console.warn('Audio play failed for remote stream', err) }
    }
  }, [stream, isLocal])
  ...
  return (
    <div
      ...
      className={cn(getTileClass(), onClick ? 'cursor-pointer hover:scale-[1.02] transition-transform' : '')}
    ...
    >
      ...
      {isRoomOwner && (
        <div className="absolute top-2 right-2 px-2 py-0.5 rounded bg-amber-500/90 text-white text-xs flex items-center gap-1"><Crown className="w-3 h-3" />Host</div>
      )}
      ...
      <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          {isMuted && <div className="p-1 rounded-full bg-destructive/80"><MicOff className="w-3 h-3 text-white" /></div>}
          {isVideoOff && <div className="p-1 rounded-full bg-destructive/80"><VideoOff className="w-3 h-3 text-white" /></div>}
        </div>
      </div>
    </div>
  )
...
const GroupVideoGrid: React.FC<GroupVideoGridProps> = ({ localStream, peerStreams, participants, currentUserId, roomCreatorId, isMuted, isVideoOff, activeSpeakerId }) => {
  const hasLocalParticipant = currentUserId ? participants.some(p => p.user_id === currentUserId) : false
  const displayParticipants = localStream && currentUserId && !hasLocalParticipant
    ? [{
        id: currentUserId,
  ...
  useEffect(() => { if (!manualPinnedRef.current) setPinnedId(activeSpeakerId || null) }, [activeSpeakerId])
  ...
  const pinnedParticipant = pinnedId ? displayParticipants.find(p => p.user_id === pinnedId) : null
  ...
  return (
    <div className={cn('grid gap-2 w-full h-full bg-background p-2 auto-rows-fr', getGridClass())}>
