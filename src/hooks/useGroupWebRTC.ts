...
export function useGroupWebRTC(userId: string | null, roomId: string | null) {
  ...
  const peerStateRef = useRef<Map<string, {
    makingOffer: boolean
    ignoreOffer: boolean
  ...
  const handleSignalRef = useRef<(from: string, data: any, messageId?: string) => Promise<void>>()
  ...
  const initLocalStream = useCallback(async () => {
    ...
    const isMobile = /Android|webOS|iPhone|iPad|iPok|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
    ...
    try {
      ...
      try {
        ...
        setMediaError('Camera and microphone access denied. Other participants won\'t see or hear you.')
        return emptyStream
      }
    }
    ...
    try {
      ...
      peersRef.current.forEach(async (peer, peerId) => {
        try {
          ...
          console.warn(`Failed to replace ${kind} track for peer ${peerId}:`, err)
        }
      })
  ...
  const reconnectPeerWithBackoff = useCallback((peerId: string) => {
    ...
    console.log(`Scheduling reconnect to ${peerId} in ${delay}ms (attempt ${attempts + 1})`)
  ...
  const createPeer = useCallback(async (peerId: string, initiator: boolean): Promise<SimplePeer.Instance> => {
    console.log(`🔗 Creating peer connection to ${peerId}, initiator: ${initiator}`)

    const existingPeer = peersRef.current.get(peerId)
    if (existingPeer) {
      closingPeersRef.current.add(peerId)
      try { existingPeer.destroy() } catch {}
    ...
    const hasTurn = iceServers.some(s => {
      return Array.isArray(s.urls)
        ? s.urls.some((u: string) => typeof u === 'string' && u.startsWith('turn'))
    ...
    console.log(`🧊 ICE config for ${peerId}: policy=${peerConfig.iceTransportPolicy}, servers=${iceServers.length}, forceRelay=${shouldForceRelay}`)
    ...
    if (pc) {
      pc.addEventListener('icecandidateerror', (ev: any) => {
        ...
        if (ignorable.includes(ev.errorCode)) {
          console.log(`🧊⚠️ ICE candidate transient error for ${peerId}: ${ev.errorCode} ${ev.errorText}`)
        } else {
          console.warn(`🧊❌ ICE candidate error for ${peerId}: ${ev.errorCode} ${ev.errorText} ${ev.url || ''}`)
        }
      })
      ...
      pc.addEventListener('iceconnectionstatechange', () => {
        ...
        if (state === 'failed' || state === 'disconnected') {
          console.log(`🧊 ICE state is ${state} for ${peerId}, scheduling reconnect`) 
    ...
    peer.on('signal', (data: any) => {
      console.log(`📤 Sending signal to ${peerId}:`, (data && data.type) || 'candidate')
      try {
    ...
    peer.on('stream', (remoteStream: MediaStream) => {
      console.log(`🎥 Received stream from ${peerId}, tracks: ${remoteStream.getTracks().map(t => `${t.kind}:${t.readyState}`).join(', ')}`)
      setPeerStreams(prev => {
    ...
    peer.on('error', (err: any) => {
      ...
      setConnectionErrors(prev => [...prev.filter(e => !e.includes(peerId)), `Connection error with ${peerId}`])
    })
  ...
  const setupAnalyserForStream = useCallback((id: string, stream: MediaStream) => {
    try {
  ...
  const handleSignal = useCallback(async (from: string, data: any, messageId?: string) => {
    if (messageId) {
  ...
  const fetchParticipants = useCallback(async () => {
    ...
    if (data) {
      ...
      const profileMap = new Map<string, { name: string; avatar_url?: string }>(
        profiles?.map((p: { user_id: string; full_name: string; profile_photo_url?: string }) => [
          p.user_id,
          { name: p.full_name, avatar_url: p.profile_photo_url }
        ] as [string, { name: string; avatar_url?: string }]) || []
      )

      setParticipants(data.map((p: { id: string; user_id: string; is_muted: boolean; is_video_off: boolean }) => {
  ...
  const joinRoom = useCallback(async () => {
    ...
    try {
      ...
      console.warn('📹 Media access failed, continuing without media:', mediaErr)
      const emptyStream = new MediaStream()
      localStreamRef.current = emptyStream
      setLocalStream(emptyStream)
    ...
    try {
      ...
      channel.on('broadcast', { event: 'signal' }, async ({ payload }: any) => {
        ...
        console.log(`📨 Received signal from ${payload.from}: ${payload.data?.type || 'candidate'}`)
        await handleSignalRef.current?.(payload.from, payload.data, payload.messageId)
      })
      ...
      channel.on('broadcast', { event: 'status_update' }, ({ payload }: any) => {
        if (!payload || payload.user_id === userIdRef.current) return
        setParticipants(prev => prev.map(p =>
          p.user_id === payload.user_id
            ? { ...p, is_muted: payload.is_muted, is_video_off: payload.is_video_off }
            : p
        ))
      })
        ...
        for (const p of existingParticipants) {
          try {
            if (userId < p.user_id && !peersRef.current.has(p.user_id)) {
              console.log(`🔗 Creating peer to existing participant ${p.user_id}`)
  ...
  }, [userId, roomId, initLocalStream, createPeer, fetchParticipants, reconnectPeerWithBackoff])
  ...
  const broadcastStatus = useCallback((isMuted: boolean, isVideoOff: boolean) => {
    if (channelRef.current && userIdRef.current && roomIdRef.current) {
      channelRef.current.send({
        type: 'broadcast',
        event: 'status_update',
        payload: { user_id: userIdRef.current, is_muted: isMuted, is_video_off: isVideoOff },
      })
      ...
      }).catch((err: any) => console.warn('Failed to update status in backend:', err))
    }
  }, [])
