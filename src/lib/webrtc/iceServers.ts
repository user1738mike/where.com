...
function dedupeServers(servers: RTCIceServer[]): RTCIceServer[] {
  ...
  for (const server of servers) {
    ...
    if (newUrls.length > 0) {
      result.push({ ...server, urls: newUrls.length === 1 ? newUrls[0] : newUrls })
    }
  }
  return result
}
...
export async function getIceServers(matchId?: string): Promise<RTCIceServer[]> {
  if (!matchId) {
    console.warn('getIceServers called without matchId — STUN fallback with public relay support')
  ...
  try {
    ...
    const { data, error } = (await Promise.race([turnPromise, timeoutPromise])) as any
