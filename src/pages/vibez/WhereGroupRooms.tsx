...
const WhereGroupRooms = () => {
  ...
  const fetchRooms = useCallback(async (suppressLoading = false) => {
    ...
    try {
      const activeSince = new Date(
        Date.now() - ACTIVE_PARTICIPANT_TIMEOUT_MS,
      ).toISOString();
  ...
  useEffect(() => {
    ...
    channel
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "group_room_participants" },
        async ({ new: newRow }: any) => {
      ...
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "public", table: "group_room_participants" },
        async ({ old: oldRow }: any) => {
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "group_room_participants" },
        async ({ old: oldRow, new: newRow }: any) => {
      ...
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "public", table: "group_rooms" },
        async () => {
          await fetchRooms(true);
        },
      )
