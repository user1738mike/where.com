    ...
    switch (action) {
      case "create": {
        ...
        if (!["public", "private"].includes(room_type)) {
          return new Response(
            JSON.stringify({
              error: 'room_type must be "public" or "private"',
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        console.log(
          `📌 Room created: ${room.id}, type: ${room_type}, host: ${user.id}`,
        );
      ...
      case "join": {
        ...
        if (existing) {
          ...
          if (existing.participant_status === "pending") {
            return new Response(
              JSON.stringify({
                error: "Your join request is pending host approval",
                status: "pending",
              }),
              {
                status: 403,
          ...
          if (existing.participant_status === "rejected") {
            return new Response(
              JSON.stringify({
                error: "Your join request was rejected by the host",
                status: "rejected",
              }),
              {
                status: 403,
        ...
        if (room.room_type === "private") {
          ...
          if (joinError) {
            console.error(
              "Join private room - create pending error:",
              joinError,
            );
            return new Response(
          ...
          console.log(
            `📌 Join request pending: ${room_id}, user: ${user.id} (private room)`,
          );
        ...
        console.log(
          `📌 Joined room: ${room_id}, user: ${user.id} (public room)`,
        );
      ...
      case "update_status": {
        ...
        if (typeof is_video_off === "boolean")
